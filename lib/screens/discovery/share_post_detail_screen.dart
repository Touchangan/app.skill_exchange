...
class SharePostDetailScreen extends StatefulWidget {
  ...
  @override
  _SharePostDetailScreenState createState() => _SharePostDetailScreenState();
}
...
class _SharePostDetailScreenState extends State<SharePostDetailScreen> {
  ...
  // 提交评论
  Future<void> _submitComment(int shareId) async {
    ...
    final url = Uri.parse("http://120.46.200.190:5500/api/shares/$shareId/comments");
    final body = {
  ...
  // 获取评论数据
  Future<List<Map<String, dynamic>>> _fetchComments(int shareId) async {
    final url = Uri.parse("http://120.46.200.190:5500/api/shares/$shareId/comments");
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final postDate = post["share_date"]; // 帖子日期
    final postImage = post["image"]; // 帖子图片
    final shareId = post["share_id"]; // 帖子 ID
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: 8),
                ElevatedButton(
                  onPressed: () => _submitComment(shareId),
                  child: Text("发送"),
                ),
              ],
            ),
